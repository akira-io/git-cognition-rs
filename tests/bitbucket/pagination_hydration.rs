...
use git_cognition::{CodeReviewsFluent, Page, PipelinesFluent, Repo, repo, run_async_test};

const NEXT_URL: &str =
    "https://api.bitbucket.org/2.0/repositories/akira-io/git-cognition-rs/items?page=2";

#[test]
fn bitbucket_repo_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
...
#[test]
fn bitbucket_code_review_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
    run_async_test(async {
...
#[test]
fn bitbucket_pipeline_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
    run_async_test(async {
