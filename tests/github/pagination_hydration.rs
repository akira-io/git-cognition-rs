...
#[test]
fn github_repo_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
...
#[test]
fn github_collaboration_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
    run_async_test(async {
        let issues = github()
            .header("link", NEXT_LINK)
            .body(r#"[{"number":42}]"#)
...
#[test]
fn github_pipeline_lists_preserve_next_cursor() -> git_cognition::CognitionResult<()> {
    run_async_test(async {
