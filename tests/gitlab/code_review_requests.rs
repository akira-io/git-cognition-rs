use git_cognition::RequestMethod;
...
#[test]
fn gitlab_code_review_delete_builds_delete_request() -> git_cognition::CognitionResult<()> {
...
fn repository() -> git_cognition::ManagedRepo<git_cognition::gitlab::GitLabProvider> {
    gitlab()
        .repo()
        .owner("akira-io")
        .name("git-cognition-rs")
...
fn code_review_resource() -> git_cognition::ManagedCodeReview<git_cognition::gitlab::GitLabProvider>
{
    gitlab().code_review().repo(repository()).id("42").get()
}

fn request_body(request: &git_cognition::Request) -> Option<&str> {
    request.body().map(git_cognition::RequestBody::as_str)
}
